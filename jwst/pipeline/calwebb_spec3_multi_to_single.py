...
class Spec3MultiPipeline(Pipeline):
    """
    ...
    class_alias = "calwebb_spec3_multi"
    ...
    def process(self, input):
        ...
        for source in sources:
            ...
            asn = asn_from_list([self.output_file+"_cal.fits"], product_name='s'+source_id)
            _, serialized = asn.dump()
            with open(f"s{source_id}_asn.json", "w") as f:
                f.write(serialized)
            output_asn_list.append(f"s{source_id}_asn.json")

        input_models.close()

        self.log.info('Ending calwebb_spec3_multi_to_single')

        kwargs = {'save_results': True}
        n_threads = cpu_count()
        pool = Pool(processes=n_threads)
        pool.map(partial(Spec3SinglePipeline.call, **kwargs), output_asn_list)
        pool.close()
        pool.join()
...
class Spec3SinglePipeline(Pipeline):
    """
    Spec3Pipeline: Processes JWST spectroscopic exposures from Level 2b to 3.
    ...
    class_alias = "calwebb_spec3_singlesource"
    ...
    def process(self, input):
        ...
        # load input members into models and ModelContainer, or just
        # do a direct open of all members in ASN file, e.g.
        input_asn = datamodels.open(input, asn_exptypes=asn_exptypes)
        input_models = datamodels.SourceModelContainer(input_asn[0])

        # Immediately update the ASNTABLE keyword value in all inputs,
        # so that all outputs get the new value
        for model in input_models:
            model.meta.asn.table_name = op.basename(input_asn.asn_table_name)
        ...
        exptype = input_models[0].meta.exposure.type
        model_type = input_models[0].meta.model_type
        output_file = input_asn.meta.asn_table.products[0].name
        self.output_file = output_file
